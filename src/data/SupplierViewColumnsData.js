import { Button } from "antd";

export const SupplierViewColumnsData = [
  {
    title: "Product img",
    key: "name",
    render: (_, record) => (
      <img
        src={
          record?.img
            ? record?.img
            : "data:image/png;base64,"
        }
        alt={record?.supplierName}
        style={{ width: 50, height: 50, borderRadius: 50 }}
      />
    ),
  },
  {
    title: "Full Name",
    dataIndex: "name",
    key: "name",
  },
  {
    title: "Price",
    key: "quanty",
    render: (_, record) => (
      <p>{record?.quanty ? record?.orginalPrice + "$" : null}</p>
    ),
  },
  {
    title: "Quantity",
    key: "price",
    render: (_, record) => (
      <p>{record?.orginalPrice ? record?.quanty + " dona" : null}</p>
    ),
  },
  {
    title: "Date",
    key: "price",
    render: (_, record) => <p>{record?.orginalPrice ? record?.date : null}</p>,
  },
  {
    title: "Status",
    key: "status",
    render: (_, record) => (
      <Button style={{ color: `${record?.status}` ? "blue" : "red" }}>
        {record?.status ? record?.status : "Aniqlanmagan"}
      </Button>
    ),
  },
];
